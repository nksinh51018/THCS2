...
$(document).ready(function(){
	...
	$("#btnSuaKhuyenMai").click(function() {
		var khuyenMaiDTO = {};
		...
		$.ajax({
			...
			success: function(value){
				if(value){
					...
					alert("Sửa khuyến mại thất bại")
				}
			}
		    })
	})
	
	$("#btnXoaKhuyenMai").click(function() {
		var id = $("#tenKhuyenMai").attr("data-value");
		$.ajax({
			url:"http://localhost:8762/khuyenMai/admin/khuyenMai/"+id,
			headers: {
		        'Authorization': "Bearer "+getCookie("tokenJWT")
		    },
			contentType: 'application/json; charset=utf-8',
			...
			success: function(value){
				if(value){
					...
					window.location.href = "/WebBanGiay2/admin/khuyenMai";
				}
				else{
					alert("Xóa khuyến mại thất bại")
				}
			}
		    })
	})
