...
import com.nk.hoadonService.dto.TongTienThangDTO;
...
public class HoaDonService {
	...
	public DanhSachTongTienThangDTO getTongTienThang() {
		...
		List<TongTienThangDTO> tongTienThangDTOs = new ArrayList<TongTienThangDTO>();
		while (k>0) {
			...
			calendar.set(nam, x-1, 1);
			int maxDay = calendar.getActualMaximum(Calendar.DAY_OF_MONTH);
			System.out.println(maxDay);
			LocalDate ngayBatDau = LocalDate.of(nam, x, 1);
			LocalDate ngayKetThuc = LocalDate.of(nam, x, maxDay);
			int kq = 0;
			try {
				kq = hoaDonDAO.getTongTienByThang(ngayBatDau, ngayKetThuc);
			}
			catch (Exception e) {
				// TODO: handle exception
				kq = 0;
			...
			TongTienThangDTO tongTienThangDTO = new TongTienThangDTO();
			tongTienThangDTO.setThang(x);
			tongTienThangDTO.setTongTien(kq);
			tongTienThangDTOs.add(tongTienThangDTO);
			x--;
			k--;
		}
		
		danhSachTongTienThangDTO.setTongTienThangs(tongTienThangDTOs);
