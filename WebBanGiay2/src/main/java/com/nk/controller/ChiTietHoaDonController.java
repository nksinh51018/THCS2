...
public class ChiTietHoaDonController {
	...
	@GetMapping("/user/chiTietHoaDon/{id}")
	public String chiTietHoaDon(@PathVariable int id,ModelMap map,HttpSession httpSession,@CookieValue(value = "tokenJWT", defaultValue = "") String token) {
		...
		if(!ten.equals("")) {
			...
			HoaDonDTO hoaDonDTO =  hoaDonService.getHoaDonById(token, id);
	...
	@GetMapping("/user/chiTietHoaDon/test/{id}")
	@ResponseBody
	public HoaDonDTO chiTietHoaDonTest(@PathVariable int id,ModelMap map,HttpSession httpSession,@CookieValue(value = "tokenJWT", defaultValue = "") String token) {
		return hoaDonService.getHoaDonById(token, id);
	}
